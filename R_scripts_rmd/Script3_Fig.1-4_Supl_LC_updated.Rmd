...
AA_Genus <- read.csv("D:/MS1_revisions/SI_Table1.csv")

AA_Genus$Abundance<- AA_Genus$Abundance*100


#removing cells <1000 
#AA_Genus2<- AA_Genus[which(AA_Genus$cells_ml>=100), ]




#Fig 3(A)
jascolor1<-    c("steelblue","#56B4E9", "lightblue1","burlywood1","burlywood3", "palevioletred2", "hotpink1", "hotpink3", "chartreuse3", "palegreen",   "snow2", "snow3", "snow4", "dimgray","tan3", "tan2", "tan", "thistle1", "thistle2", "plum1", "plum3","plum4")  #Picking colors
#"chartreuse3", "chartreuse4"
  ...
  yv1a_CC2 <-        ggplot(data = AA_Genus,
                            mapping = aes_string(x ="Sample",fill="Phylum",y="Abundance"))+
    ...
    theme(legend.box.background = element_rect(color="white", size=2), panel.border = element_rect(colour = "black", fill=NA, size=2))+
     ...
     ggtitle("A. Relative Abundance of Prokaryotic Phyla" )+
 ...
 yv1a_CC <-        ggplot(data = AA_Genus,
                            mapping = aes_string(x ="Sample",fill="Phylum",y="cells_ml"))+
    geom_bar(stat="identity")+
    theme(legend.position="none")+
    ...
    theme(legend.box.background = element_rect(color="white", size=2), panel.border = element_rect(colour = "black", fill=NA, size=2))+
      theme(axis.text = element_text(size = 12))+
     guides(fill=guide_legend(nrow = 11, title=" A. Phylum"))+
     ggtitle("B. Absolute Abundance of Prokaryotic Phyla" )+
   theme(axis.text=element_text(size=14),
          axis.title=element_text(size=14,face="bold"), legend.text = element_text(size=12))+
        xlab("Depth (m)")+
    theme(
 ...
 #Fig 3(B)
  ...
  Phylum_proteobacteria<- Phylum_proteobacteria[which(Phylum_proteobacteria$cells_ml>=1000), ]
 
   ProtOTU2_P <-        ggplot(data = Phylum_proteobacteria,
                   mapping = aes_string(x ="Sample",y="cells_ml",fill="Genus"))+
                   geom_bar(stat="identity")+
                   ...
                   ggtitle("C. Inferred Absolute Abundance of Proteobacteria (Genus)" )+
                   theme(legend.position="right")+
                   theme(axis.text=element_text(size=14),
  ...
  theme(plot.title = element_text(size = 16, face = "bold"))+
  theme(legend.position = c(0.74, 0.50), legend.direction = "vertical",
                   legend.background = element_rect(size=0.3, 
                                   colour ="transparent", fill = "transparent"), legend.text = element_text(color = "black"))+
guides(fill=guide_legend(nrow=26, title= "C. Genus"))+
  theme(legend.title=element_text(size=14, face = "bold"), legend.text=element_text(size=14))+
   ...
   Phylum_Bacteroidetes<- AA_Genus[which(AA_Genus$Phylum=="Bacteroidetes"), ]
    Phylum_Bacteroidetes<- Phylum_Bacteroidetes[which(Phylum_Bacteroidetes$cells_ml>=1000), ]
    ...
    ProtOTU2_B <-        ggplot(data = Phylum_Bacteroidetes,
                   ...
                   ggtitle("D. Inferred Absolute Abundance of Bacteroidetes (Genus)" )+
                   theme(legend.position="right")+
                   theme(axis.text=element_text(size=14),
                   axis.title=element_text(size=14,face="bold"))+
                   xlab("")+
  ...
  theme(plot.title = element_text(size = 16, face = "bold"))+
  theme(legend.position = c(0.70, 0.50), legend.direction = "vertical",
                   legend.background = element_rect(size=0.3, 
                                   colour ="transparent", fill = "transparent"), legend.text = element_text(color = "black"))+
guides(fill=guide_legend(ncol=1, title="D. Genus" ))+     
  theme(legend.title=element_text(size=14, face = "bold"), legend.text=element_text(size=14))+
              scale_y_continuous(labels = scales::scientific)+
           theme (legend.box.background = element_rect(colour = "black"))+
           scale_y_continuous(labels = scales::scientific, breaks = scales::pretty_breaks(n = 6), limits = c(0, 9e+05))
  ...
  (yv1a_CC2 | yv1a_CC) / (ProtOTU2_P | ProtOTU2_B)
...
table_cyano<- AA_Genus[which(AA_Genus$Phylum=="Cyanobacteria"), ]
...
#Fig 4 (B)
     #uahbs234ab_cyna_CPF2<- table_cyano[which(table_cyano$cells_ml>=100), ]


 Phylum_cyano2abn_CC <-        ggplot(data = table_cyano,
                            mapping = aes_string(x ="Sample",y="cells_ml",fill="Genus"))+
    geom_bar(stat="identity")+
    theme(legend.position="right")+
...
#    endsWith(Genus, "Pseudanabaena_PCC-7429") ~ "Non-chloroplast"))
 ...
 cyano_Cells2 = dplyr::mutate(table_cyano, TAX = paste(OTU, Genus,  sep='|'))
 ...
 cyano_Cells2$TAX <- factor(cyano_Cells2$TAX, levels = c("Otu00052|Chloroplast_ge", "Otu00042|Chloroplast_ge", "Otu00051|Chloroplast_ge", "Otu00033|Chloroplast_ge","Otu00055|Chloroplast_ge", "Otu00143|Chloroplast_ge",               "Otu00208|Chloroplast_ge","Otu00006|Chloroplast_ge", "Otu00323|Oxyphotobacteria_unclassified", "Otu00342|Oxyphotobacteria_unclassified", "Otu00008|Oxyphotobacteria_unclassified", "Otu00475|Gastranaerophilales_ge", "Otu00106|Cyanobium_PCC-6307", "Otu00286|Pseudanabaena_PCC-7429"))
     ...
     newOTU1BN <-        ggplot(data = cyano_Cells2,
                            mapping = aes_string(x ="Sample",y="cells_ml",fill="TAX"))+
    theme(legend.position="right")+
        geom_bar(stat="identity")+
     ylab("Absolute Abundance of Chloroplast (Cells/ml)")+
    theme(axis.text = element_text(size = 14))+
...
"Y17632.1.2184","CP003940.1433867.1435344", "CP003940.1167849.1169324",   "Otu00006", "Otu00008" ,"Otu00033", "Otu00042", "Otu00051", "Otu00052", "Otu00055", "Otu00106", "Otu00143",  "Otu00208", "Otu00286", "Otu00323" , "Otu00342", "Otu00475")
 ...
 OTUdf<- data.frame(OTU)
   ...
   withotudf<- merge(meta_sheet12, OtudfT, by= "Accession_ID", all=T)
     ...
     OTU_ID_cya<- table_cyano %>%                           #Cyanobacteria AA 16s
     dplyr::  select("Sample", "Genus", "cells_ml", "OTU")
...
Figure4<- ggarrange(yv1abz_Phylum_cyano2abn_CC, newOTU1BN, p_c, ncol=3, widths = c(0.6,1,0.40), align = "h")
...
Fig.S4<- ggarrange(VLP_oxy, VLP_par, VLP_dep, VLP_cond, VLP_SP, NULL, nrow = 3, ncol= 2, common.legend = T, legend= "bottom", labels = c("(A)", "(B)", "(C)"," (D)", "(E)", "(F)"))
 ...
 palette1 <- distinctColorPalette(33)
    ...
    Relative_Genus<- AA_Genus2[which(AA_Genus2$cells_ml>=5000), ]

    pieipe3_ABN_Genus <-        ggplot(data = Relative_Genus,
                          mapping = aes_string(x=  "Sample", y="cells_ml", fill="TAX"))+ theme_bw()+
     geom_bar(stat="identity")+
    theme(legend.position="right")+ 
