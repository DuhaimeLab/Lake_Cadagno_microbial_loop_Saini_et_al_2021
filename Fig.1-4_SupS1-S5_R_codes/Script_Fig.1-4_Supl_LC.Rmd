...
#Primary production
 finalPP_chl23 <- read.csv("~/GitHub/Lake_Cadagno_microbial_loop_Saini_et_al_2021/Fig.1-4_Suppl/PP_R.csv") 
...
AA_Genus <- read.csv("~/GitHub/Lake_Cadagno_microbial_loop_Saini_et_al_2021/Fig.1-4_Suppl/AA_at_Genus.csv")

#Absolute Abundance at Phylum 
gpsfSI2 <- read.csv("~/GitHub/Lake_Cadagno_microbial_loop_Saini_et_al_2021/Fig.1-4_Suppl/AA_at_Phylum.csv")
...
guides(fill=guide_legend(nrow=13, title="C. Genus" ))+     
 ...
 palette1 <- distinctColorPalette(68)
  ...
  theme(legend.title=element_text(size=12, face="bold"), 
    ...
    fig.S3<- pieipe3_ABN | pieipe3_ABN_Genus
    fig.S3
...
#16s Cyanobacteria Absolute Abundance
 uahbs234ab_cyna_CPF <- read.csv("~/Cyanobacteria_AA.csv") 
...
Phylum_cyano2abn_CC <-        ggplot(data = uahbs234ab_cyna_CPF,
  ...
  theme(legend.title=element_text(size=24),
    ...
    legend.text=element_text(size=24))+
     scale_y_continuous(labels = scales::scientific)+
      ...
      theme(legend.key.size = unit(2.5, 'lines'))
       ...
       uahbs234ab_cyna_CP3$OTU <- factor(uahbs234ab_cyna_CP3$OTU, levels = c("Otu00052", "Otu00042", "Otu00051", "Otu00033", "Otu00055", "Otu00143", "Otu00208", "Otu00006"))
    ...
    coord_flip()+                   theme_bw()+ scale_fill_manual(values = c("darkorange3", "darkorange2", "darkorange1", "darkorchid4", "darkorchid3", "darkolivegreen", "darkolivegreen4", "darkolivegreen3"))+
    xlab("Depth (m)")+
    theme(legend.position = "left")+
      theme(axis.text = element_text(size = 14))+
...
VLP_cor2 <- Meta_data_PLPII[-c(3,4), ]
  ...
  stat_cor(method = "pearson", label.y = 2e+08, label.x = 5e-07)+ theme_bw()+
    ...
    ylab("VLP/mL")+
     ...
     scale_color_manual("Depth (m)", values=c("darkseagreen","forestgreen", "darkolivegreen3", "mediumorchid1", "deeppink3", "deeppink4", "peachpuff2", "peachpuff4"))
        ...
        Fig.S2<-     ggarrange(VLP_oxy, VLP_par, VLP_dep, VLP_cond, VLP_SP, NULL, nrow = 3, ncol = 2, common.legend = T, legend= "bottom", labels = c("(A)", "(B)", "(C)"," (D)", "(E)"))
        Fig.S2
...
oxygen_pcoa<- ggplot(Meta_data_PLP_VLP_II, aes(CTD_28_29_Oxygen, Axis.1_16s_b) ) +
  ...
  stat_cor(method = "pearson", label.y = 0.5, label.x = 1)+ theme_bw()+
     labs(title = "A. Oxygen vs Bacterial alpha diversity (incl. Chloroplast)", subtitle = "Local Polynomial Regression, Method=LM")+
    xlab("Day 1 and Day 2 Oxygen (mg/L)")+
  ...
  stat_cor(method = "pearson", label.y = 0.5, label.x = 3)+ theme_bw() +
        labs(title = "B. Light vs Bacterial alpha diversity (incl. Chloroplast)", subtitle = "Local Polynomial Regression, Method=LM")+
    xlab("Day 2 PAR (w/m^2)")+
        ylab("PCoA Axis.1 Variation (16s rRNA)")+
      # theme(axis.text=element_text(size=24),axis.title=element_text(size=24,face="bold"))+
     #
geom_point(aes(color=as.factor(depth_m)), size=4)+
  ...
  stat_smooth(color = "black", size=0.3, method="lm") +
  stat_cor(method = "pearson", label.y = 0.5, label.x = 1)+ theme_bw()+
     labs(title = "C. Oxygen vs Phenotypic PLP Diversity", subtitle = "Local Polynomial Regression, Method=LM")+
    xlab("Day 1 and Day 2 Oxygen (mg/l)")+
...
light_pcoa2<- ggplot(Meta_data_PLP_VLP_II, aes(CTD2_PAR_29, PCoA_Cells_SYBR_Axis1)) +
  ...
  stat_smooth(color = "black", size=0.3, method="lm") +
  stat_cor(method = "pearson", label.y = 0.5, label.x = 3)+ theme_bw() +
        labs(title = "D. Light vs Phenotypic PLP Diversity", subtitle = "Local Polynomial Regression, Method=LM")+
    xlab("Day 2 PAR (w/m^2)")+
        ylab("PCoA Axis.1 Variation (Flow-Cytometry)")+
      # theme(axis.text=element_text(size=24),axis.title=element_text(size=24,face="bold"))+
     #
geom_point(aes(color=as.factor(depth_m)), size=4)+
  ...
  stat_cor(method = "pearson", label.y = -0.4, label.x = -0.2)+ theme_bw() +
       labs(title = "E. Phenotypic vs Genotypic Prokaryotic Beta Diversity", subtitle = "Linear Regression, Method=LM")+
  ...
  stat_cor(method = "pearson", label.y = 10000, label.x = 60)+ theme_bw()+
        scale_color_manual("Depth (m)", values=c("#52854C", "#C3D7A4","darkseagreen","forestgreen", "darkolivegreen3", "mediumorchid1", "deeppink4", "deeppink1", "#CC79A7", "peachpuff2", "peachpuff4"))+
             geom_point(aes(color=as.factor(depth_m)), size=4)+
       labs(title = "F. Phenotypic vs Genotypic Prokaryotic Alpha Diversity", subtitle = "Linear Regression, Method=LM")+
       xlab("Fisher Diversity (16s rRNA)")+
        ylab("Phenotypic Diversity, D1 (Flow-cytometry)")
       
        
      Fig.S5<-     ggarrange(oxygen_pcoa, light_pcoa, oxygen_pcoa2bv, light_pcoa2, geno_p, geno_p2, nrow = 3, ncol = 2, common.legend = T, legend= "bottom")
      Fig.S5
  ...
  stat_cor(method = "pearson", label.y = 12, label.x = 1)+ theme_bw()+
    ...
    theme(axis.ticks.y = element_blank())
        ...
        Fig.S6<-     (Lentilm_L |  Lentilm_E| Lentilm_CL)/ (LentiT_L| LentiT_E| LentiT_CL)   + plot_layout(guides = "collect") +
  plot_annotation(tag_levels = 'A',  tag_prefix = '(',
                         tag_suffix = ')') & 
