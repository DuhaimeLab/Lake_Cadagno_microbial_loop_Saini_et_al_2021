...
TurbvOxy123<- ggplot(data=CTD1_28_Aug_17_R, aes(y=CTD1_28_Aug_17_R$Depth_m_CTD1_28))+
     ...
     geom_path(data=CTD1_29_Aug_17_avg_R, aes(y=as.numeric(CTD1_29_Aug_17_avg_R$Depth), x=CTD1_29_Aug_17_avg$Oxy_correc*2.5), 
...
chlpar<- ggplot(data=CTD1_29_Aug_17_R, aes(x=Depth_m_CTD1_29)) + 
   ...
   geom_path(data=CTD2_29_Aug_17, aes(y=as.numeric(Depth_m_CTD2_29), x=CTD2_29_Aug_17$Phycocyanin_CTD2_29*10), color="lightsalmon3", linetype="solid", size=1.5)+
...
#Fig.1 (F) 
 ...
 geom_point(data=finalPP_chl23%>% dplyr:: filter(Reaction=="Light"), aes(y = log(netPP), colour="Net"), size=3.5)+
  geom_line(data=finalPP_chl23%>% dplyr::filter(Reaction=="Light"), aes(y = log(netPP), x=Depth),linetype = "dashed", colour="darkolivegreen4", size=2)
...
figure1_gg / figure1b_gg #22x24 pdf
...
#removing cells <1000 
AA_Genus2<- AA_Genus[which(AA_Genus$AA_Arch_Bac>=1000), ]
...
#Fig 3(A)
jascolor1<-    c("steelblue","#56B4E9", "lightblue1","burlywood1","burlywood3","chartreuse3", "palegreen",   "snow2", "snow3", "snow4", "dimgray", "thistle1", "thistle2", "plum1", "tan2", "tan3")  #Picking colors
...
guides(fill=guide_legend(nrow=13, title="C. Genus" ))+     
  ...
  #fig.3  
  figure3<-   (yv1a_CC / (ProtOTU2_P | ProtOTU2_B)) #15 x15.5
...
yv1a_CC_Cyano <-        ggplot(data = uahbs234ab_cyna2,
    ...
    theme(legend.box.background = element_rect(color="white", size=2), panel.border = element_rect(colour = "black", fill=NA, size=2))+
      theme(axis.text = element_text(size = 12))+
     guides(fill=guide_legend(nrow = 8, title=" A. Phylum"))+
     ggtitle("A. Absolute Abundance (Cyanobacteria Without Putative Chloroplasts)" )+
   theme(axis.text=element_text(size=14),
          axis.title=element_text(size=14,face="bold"), legend.text = element_text(size=12))+
        xlab("Depth (m)")+
    theme(
...
phy_oxygen<- ggplot(Meta_data_PLP_VLP_II, aes(y=Meta_data_PLP_VLP_II$CTD2_Phycocyanin_29, x=Meta_data_PLP_VLP_II$CTD_28_29_Oxygen)) +
 ...
 Fig.S3<-     ggarrange(Sp_chl, Sp_phy, Sp_bio, Sp_oxy, SP_par, Sp_cells, chl_par, chl_oxygen, phy_par, phy_oxygen, nrow = 5, ncol = 2, common.legend = T, legend= "bottom", labels = c("(A)", "(B)", "(C)"," (D)", "(E)", "(F)", "(G)", "(H)", "(I)", "(J)"), align = "v") #15x15 pdf
...
Fig.S4<-     ggarrange(PLP_turb, PLP_chl, PLP_POC, PLP_POS, PLP_PON, NULL, nrow = 3, ncol = 2, common.legend = T, legend= "bottom", labels = c("(A)", "(B)", "(C)"," (D)", "(E)"))
Fig.S4 
...
Fig.S5<- ggarrange(VLP_oxy, VLP_par, VLP_dep, VLP_cond, VLP_SP, NULL, nrow = 3, ncol= 2, common.legend = T, legend= "bottom", labels = c("(A)", "(B)", "(C)"," (D)", "(E)", "(F)"))
        Fig.S5
 ...
 pieipe3_ABN <-        ggplot(data = AA_Genus,
     ...
     geom_bar(stat="identity")+
    theme(legend.position="right")+ 
     scale_fill_manual(values = c("#EBB155","#E363CE","#DBE4A2",  "#80ACE3", "#D7A6C8", "#816FD2", "#72EA4F", "#6B9198", "#7C3EE0", "#959F5F", "#DCD6E1", "#DBE555", "#DB5D8E", "#7BE081", "#79DCE4", "#E09C86", "#C8E1CA", "#6FDBB4", "#CA8FD9", "#D542E5"))+
    theme(legend.box.background = element_rect(color="white", size=2), panel.border = element_rect(colour = "black", fill=NA, size=2))+
      theme(axis.text = element_text(size = 12))+
 ...
 palette1 <- distinctColorPalette(33)
  ...
  theme(legend.title=element_text(size=12, face="bold"), 
    ...
    fig.S6<- pieipe3_ABN | pieipe3_ABN_Genus
    fig.S6
...
#Fig.S7
  ...
  stat_cor(method = "pearson", label.y = 10, label.x = 2)+ theme_bw()+
    ...
    ylab("Day 1 and Day 2 Average Oxygen (mg/L)")+
     ...
     Fig.S7<-     ggarrange(turb_chro, PLP_chro, Lentilm_L, turb_lenti,  PLP_lenti, Lentilm_PON, Lentilm_POS, Lentilm_ligh, Lentilm_ox, nrow = 5, ncol = 2, common.legend = T, legend= "bottom", labels = c("(A)", "(B)", "(C)"," (D)", "(E)", "(F)", "(G)", "(H)", "(I)"), align="v") #15x15 pdf
  ...
  Fig.S7<-     ggarrange(Lentilm_L, Lentilm_T, Lentilm_PON, Lentilm_POS, Lentilm_ox, Lentilm_ligh, Lentilm_E, Lentilm_CL, Lentilm_Ery, nrow = 5, ncol = 2, common.legend = T, align = "hv", legend= "bottom", labels = c("(A)", "(B)", "(C)"," (D)", "(E)", "(F)", "(G)", "(H)"))   
...
#Fig.S8
 ...
 Fig.S8<-     ggarrange(PLPD_turb, PLPD_BP, PLPD_chl, PLPD_ox, PLPD_PAR, nrow = 3, ncol = 2, common.legend = T, legend= "bottom", labels = c("(A)", "(B)", "(C)"," (D)", "(E)", "(F)"))
...
geom_point(aes(color=as.factor(Depth_m)), size=4)+
  ...
  stat_cor(method = "pearson", label.y = -0.4, label.x = -0.2)+ theme_bw() +
      ...
      Fig.S9<-     ggarrange(PLPD_ox, PLPD_PAR, oxygen_pcoa, light_pcoa, oxygen_pcoa2bv, light_pcoa2, geno_p2, geno_p, nrow = 4, ncol = 2, common.legend = T, legend= "bottom")
      Fig.S9
 
      ###########################
Meta_data_PLP_VLP_II$OTU_RA.Bacteroidetes.Lentimicrobiaceae_ge
Meta_data_PLP_VLP_II$Bacterial_production
Meta_data_PLP_VLP_II$OTU_RA.Proteobacteria.Chromatium

Meta_data_PLP_VLP_II$OTU_RA.Firmicutes.Erysipelotrichaceae_unclassified
Meta_data_PLP_VLP_II$OTU_RA.Cyanobacteria.Chloroplast_ge
Meta_data_PLP_VLP_II$
